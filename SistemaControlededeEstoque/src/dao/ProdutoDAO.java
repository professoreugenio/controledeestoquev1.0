...
public class ProdutoDAO {
	...
	public boolean cadastrar(Produto produto) {
		...
		String sql = "INSERT INTO produtos "
				+ "(nome, descricao, valor_custo, valor_venda, quantidade_estoque, estoque_minimo, id_categoria, id_fornecedor, foto) "
				+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";

		try (Connection con = ConexaoDAO.conectar(); PreparedStatement stmt = con.prepareStatement(sql)) {
	...
	public List<Produto> listar() {
		...
		String sql = "SELECT " + "p.id_produto, " + "p.nome, " + "p.descricao, " + "p.valor_custo, " + "p.ativo, "
				+ "p.valor_venda, " + "p.quantidade_estoque, " + "p.estoque_minimo, " + "c.nome AS categoria, "
				+ "f.nome AS fornecedor " + "FROM produtos p "
				+ "INNER JOIN categorias c ON p.id_categoria = c.id_categoria "
				+ "LEFT JOIN fornecedores f ON p.id_fornecedor = f.id_fornecedor " + "WHERE p.ativo = 1 "
				+ "ORDER BY p.nome";
	...
	public List<Produto> pesquisarPorNome(String nomePesquisa) {
		...
		String sql = "SELECT " + "p.id_produto, " + "p.nome, " + "p.descricao, " + "p.valor_custo, " + "p.valor_venda, "
				+ "p.quantidade_estoque, " + "p.estoque_minimo, " + "c.nome AS categoria, " + "f.nome AS fornecedor "
				+ "FROM produtos p " + "INNER JOIN categorias c ON p.id_categoria = c.id_categoria "
				+ "LEFT JOIN fornecedores f ON p.id_fornecedor = f.id_fornecedor " + "WHERE p.ativo = 1 "
				+ "AND p.nome LIKE ? " + "ORDER BY p.nome";
	...
	public Produto buscarPorId(int idProduto) {
		...
		String sql = "SELECT id_produto, nome, descricao, valor_custo, valor_venda, "
				+ "quantidade_estoque, estoque_minimo, id_categoria, id_fornecedor, foto " + "FROM produtos "
				+ "WHERE id_produto = ? " ;
	...
	public boolean atualizar(Produto produto) {
		...
		String sql = "UPDATE produtos SET " + "nome = ?, " + "descricao = ?, " + "valor_custo = ?, "
				+ "valor_venda = ?, " + "quantidade_estoque = ?, " + "estoque_minimo = ?, " + "id_categoria = ?, "
				+ "id_fornecedor = ?, " + "foto = ? " + "WHERE id_produto = ?";
	...
	public List<Produto> listarParaCombo() {
		...
		String sql = "SELECT id_produto, nome, valor_custo, valor_venda, quantidade_estoque " + "FROM produtos "
		...
		try (Connection con = ConexaoDAO.conectar();
			...
			System.out.println("Erro ao listar produtos para combo: " + erro.getMessage());
		}
	...
	public int buscarEstoqueAtual(int idProduto) {
		...
		String sql = "SELECT quantidade_estoque FROM produtos WHERE id_produto = ?";
		...
		try (Connection con = ConexaoDAO.conectar(); PreparedStatement stmt = con.prepareStatement(sql)) {
			...
			System.out.println("Erro ao buscar estoque atual: " + erro.getMessage());
		}
	...
	public List<Produto> listarEstoque() {
		...
		String sql = "SELECT " + "p.id_produto, " + "p.nome, " + "p.foto, " + "p.valor_venda, "
				+ "p.quantidade_estoque, " + "p.estoque_minimo, " + "c.nome AS categoria, " + "f.nome AS fornecedor "
				+ "FROM produtos p " + "INNER JOIN categorias c ON p.id_categoria = c.id_categoria "
				+ "LEFT JOIN fornecedores f ON p.id_fornecedor = f.id_fornecedor " + "WHERE p.ativo = 1 "
				+ "ORDER BY p.nome";
		...
		try (Connection con = ConexaoDAO.conectar();
			...
			System.out.println("Erro ao listar estoque: " + erro.getMessage());
		}

		return produtos;
	}

	public List<Produto> pesquisarEstoquePorNome(String nomePesquisa) {

		List<Produto> produtos = new ArrayList<>();

		String sql = "SELECT " + "p.id_produto, " + "p.nome, " + "p.valor_venda, " + "p.quantidade_estoque, "
				+ "p.estoque_minimo, " + "c.nome AS categoria, " + "f.nome AS fornecedor " + "FROM produtos p "
				+ "INNER JOIN categorias c ON p.id_categoria = c.id_categoria "
				+ "LEFT JOIN fornecedores f ON p.id_fornecedor = f.id_fornecedor " + "WHERE p.ativo = 1 "
				+ "AND p.nome LIKE ? " + "ORDER BY p.nome";
		...
		try (Connection con = ConexaoDAO.conectar(); PreparedStatement stmt = con.prepareStatement(sql)) {
			...
			System.out.println("Erro ao pesquisar estoque: " + erro.getMessage());
		}
